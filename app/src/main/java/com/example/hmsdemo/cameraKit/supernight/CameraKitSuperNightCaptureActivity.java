/* CameraKitSuperNightCaptureActivity.java */
 ...
 * Demo Activity
 */
public class CameraKitSuperNightCaptureActivity extends BaseActivity {
    ...
    private void initManualIsoSpinner() {
        ...
        if ((parameters != null) && (parameters.contains(RequestKey.HW_SUPER_NIGHT_ISO))) {
            List<Long> lists = mModeCharacteristics.getParameterRange(RequestKey.HW_SUPER_NIGHT_ISO);
            ranges = new Long[lists.size()];
            lists.toArray(ranges);
        }
        initSpinner(R.id.manualIso, longToList(ranges, R.string.manualIso), new SpinnerOperation() {
            @Override
            public void doOperation(String text) {
                try {
                    mMode.setParameter(RequestKey.HW_SUPER_NIGHT_ISO, Long.parseLong(text.split(PIVOT)[1]));
                } catch (PatternSyntaxException | NumberFormatException e) {
                    Log.e(TAG, "patternSyntaxException NumberFormatException text: " + text);
                }
            }
        });
    }

    private void initManualExposureSpinner() {
        Long[] ranges = new Long[0];
        List<CaptureRequest.Key<?>> parameters = mModeCharacteristics.getSupportedParameters();
        if ((parameters != null) && (parameters.contains(RequestKey.HW_SUPER_NIGHT_EXPOSURE))) {
            List<Long> lists = mModeCharacteristics.getParameterRange(RequestKey.HW_SUPER_NIGHT_EXPOSURE);
            ranges = new Long[lists.size()];
            lists.toArray(ranges);
        }
        initSpinner(R.id.manualExposure, longToList(ranges, R.string.manualExposure), new SpinnerOperation() {
            @Override
            public void doOperation(String text) {
                try {
                    mMode.setParameter(RequestKey.HW_SUPER_NIGHT_EXPOSURE, Long.parseLong(text.split(PIVOT)[1]));
                } catch (PatternSyntaxException | NumberFormatException e) {
                    Log.e(TAG, "patternSyntaxException NumberFormatException text: " + text);
                }
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_camera_supernight);
